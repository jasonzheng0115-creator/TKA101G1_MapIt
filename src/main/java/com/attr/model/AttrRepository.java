...
public interface AttrRepository extends JpaRepository<AttrVO, Integer>, JpaSpecificationExecutor<AttrVO> {
     ...
     * @return 符合條件的景點分頁資料
     */
    Page<AttrVO> findByRegionVO_RegionIdAndCategoryVO_CategoryId(Integer regionId, Integer categoryId,
            Pageable pageable);
     ...
     * @return 符合條件的景點分頁資料
     */
    Page<AttrVO> findByAttrNameContainingAndCategoryVO_CategoryId(String keyword, Integer categoryId,
            Pageable pageable);
    ...
    /**
     * 同時比對關鍵字與「景點名稱」、「地區名稱」或「類別名稱」
     * 
     * @param keyword  搜尋關鍵字
     * @param pageable 分頁參數
     * @return 符合任一條件的景點分頁資料
     */
    @Query("SELECT a FROM AttrVO a WHERE a.attrName LIKE %:keyword% " +
            "OR a.regionVO.regionName LIKE %:keyword% " +
            "OR a.categoryVO.categoryName LIKE %:keyword%")
    Page<AttrVO> findByKeywordCombined(@Param("keyword") String keyword, Pageable pageable);
